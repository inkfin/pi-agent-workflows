...
多机同步采用双层结构：**每个项目一个 jj tree repo**（`<project>/.pi/tree/`，git-backed，各自 remote，存 GraphTransactions、session 快照与 Attachment payload）；**一个全局 registry repo**（XDG 数据目录，单一中心 remote，只存项目/会话元数据，不存内容）。全局可见性在 UX 层整合：pull registry → 展示跨机 session 列表 → 按需从对应项目 remote 拉取快照。
...
ADR-0004/0006 细化：每 origin 独立 bookmark `grove/origins/<originId>`；
frontier 保存 semantic head nodeIds，而 bookmark/jj DAG 保证所有 GraphTransactions
物理可达；Registry 最终一致 + outbox；同步默认关闭。
